...
export default function InvoicesPage() {
    ...
    const toggleStatus = (id: string) => {
        ...
        if (!inv) return;
        const next = inv.status === 'draft' ? 'sent' : inv.status === 'sent' ? 'paid' : inv.status === 'paid' ? 'draft' : 'paid';
        erp.updateInvoiceStatus(id, next);
    };
                        ...
                        {filtered.length === 0 ? (
                            ...
                            return (
                                <tr key={inv.id} className="hover:bg-slate-50">
                                    ...
                                    <td className="px-4 py-3 text-center">
                                        <button onClick={() => erp.removeInvoice(inv.id)} className="text-slate-300 hover:text-red-500 transition"><Trash2 className="w-4 h-4" /></button>
                                    </td>
                                </tr>
                            );
                        })}
