    ...
    return (
        <div className="max-w-4xl mx-auto pb-20 space-y-8">
            ...
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h1 className="text-xl font-bold text-slate-900">Phòng ban & Kênh Marketing</h1>
                    <p className="text-xs text-slate-500 mt-0.5">Chia Payroll Pool và Marketing Pool cho từng đơn vị</p>
                </div>
            </div>
            ...
            <section>
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-bold text-slate-700">
                            Phòng ban (Payroll Pool: {formatVND(payrollPool)})
                        </h2>
                    </div>
                    <button
                ...
                {totalDeptPercent > 0 && Math.abs(totalDeptPercent - 100) > 0.01 && (
                    <div className="mb-3 bg-amber-50 border border-amber-200 text-amber-700 text-sm rounded-xl px-4 py-3 flex items-center gap-2">
                        ...
                        Tổng phân bổ phòng ban = {totalDeptPercent.toFixed(1)}% (nên = 100% Payroll Pool)
                    </div>
                )}
                ...
                {showDeptForm && (
                    <div className="mb-4 bg-white rounded-xl border border-slate-200 p-4 space-y-3">
                        <div className="grid grid-cols-2 gap-3">
                            <input
                                placeholder="Tên phòng ban (VD: Sales)"
                                value={deptName}
                        ...
                        <table className="w-full text-sm">
                                ...
                                {departmentBudgets.map(db => (
                                    <tr key={db.department.id} className="hover:bg-slate-50/50">
                                        ...
                                        <td className="px-5 py-3">
                                            <button
                                                onClick={() => {
                                                    if (confirm(`Xóa phòng ban "${db.department.name}"? Tất cả nhân viên trong phòng ban cũng sẽ bị xóa.`))
                            ...
                            <tfoot>
                                <tr className="bg-slate-50 border-t border-slate-200">
                                    <td className="px-5 py-3 font-bold text-slate-700">Tổng</td>
                                    <td className="px-5 py-3 text-center">
            ...
            <section>
                <div className="flex items-center justify-between mb-4">
                    <div className="flex items-center gap-2">
                        ...
                        <h2 className="text-sm font-bold text-slate-700">
                            Kênh Marketing (Pool: {formatVND(marketingPool)})
                        </h2>
                    </div>
                    <button
                ...
                {totalChannelPercent > 0 && Math.abs(totalChannelPercent - 100) > 0.01 && (
                    <div className="mb-3 bg-amber-50 border border-amber-200 text-amber-700 text-sm rounded-xl px-4 py-3 flex items-center gap-2">
                        <AlertTriangle className="w-4 h-4 shrink-0" />
                        Tổng phân bổ kênh = {totalChannelPercent.toFixed(1)}% (nên = 100% Marketing Pool)
                    </div>
                )}
                ...
                {showChForm && (
                    <div className="mb-4 bg-white rounded-xl border border-slate-200 p-4 space-y-3">
                        <div className="grid grid-cols-2 gap-3">
                            <input
                                placeholder="Tên kênh (VD: Facebook Ads)"
                                value={chName}
                                ...
                                {channelBudgets.map(cb => (
                                    <tr key={cb.channel.id} className="hover:bg-slate-50/50">
                                        ...
                                        <td className="px-5 py-3">
                                            <button
                                                onClick={() => {
                                                    if (confirm(`Xóa kênh "${cb.channel.name}"?`))
