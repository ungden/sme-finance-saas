...
interface FinanceContextType {
    ...
    // Plan YearData: full annual financial plan generated by AI Planner
    planYearData: YearData | null;
    setPlanYearData: (data: YearData | null) => void;
...
export function FinanceProvider({ children }: { children: ReactNode }) {
    ...
    const [planYearData, setPlanYearData] = useState<YearData | null>(null);
    ...
    useEffect(() => {
        ...
        // Load saved plan year data
        const savedPlan = localStorage.getItem("sme_finance_plan_year_data");
        if (savedPlan) {
            try { setPlanYearData(JSON.parse(savedPlan)); } catch { /* ignore */ }
        }
    ...
    // Persist plan year data
    useEffect(() => {
        if (!isLoaded) return;
        if (planYearData) {
            localStorage.setItem("sme_finance_plan_year_data", JSON.stringify(planYearData));
        } else {
            localStorage.removeItem("sme_finance_plan_year_data");
        }
    }, [planYearData, isLoaded]);
